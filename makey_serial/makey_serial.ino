 ...
 ************************************************
 ...
 NB: the keyboard & mouse are both disabled.
 Instead the information is sent through the serial interface.
 This way, no window is required to be in focus and data can be continually processed.
 */
      ...
      if (inputs[i].isMouseButton) {
        if (inputs[i].pressed) {
          ...
          if (inputs[i].keyCode == MOUSE_LEFT) {
            ...
            Serial.println('release mouse left');
          } 
          if (inputs[i].keyCode == MOUSE_RIGHT) {
            //Mouse.release(MOUSE_RIGHT);
            Serial.println('release mouse right');
          }           
        }
      }
    }
...
void sendMouseMovementEvents() {
  ...
  if (mouseMovementCounter == 0) {
    ...
    if( !((horizmotion == 0) && (vertmotion==0)) )
    {
      ...
      Serial.print("mouse ");
      Serial.print(horizmotion * PIXELS_PER_MOUSE_STEP);
      Serial.print(",");
      Serial.println(vertmotion * PIXELS_PER_MOUSE_STEP);
    }
  }
}
